...
{
    if (pGraphics == NULL) return OC_INVALID_ARGS;
...
{
    if (pSwapchain == NULL) return OC_INVALID_ARGS;
    ocZeroObject(pSwapchain);

    if (pGraphics == NULL || pWindow == NULL) return OC_INVALID_ARGS;
...
{
    if (pRT == NULL) return OC_INVALID_ARGS;
    ocZeroObject(pRT);
